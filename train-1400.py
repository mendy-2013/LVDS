...
from model5 import LDVS
from cal_ssim import SSIM
from gpu_select import Auto_Select_GPU
Auto_Select_GPU()
...
class Session:
    def __init__(self):
        ...
        # self.opt_net=torch.optim.RMSprop(self.net.parameters(),lr=settings.lr,alpha=0.9)
        self.sche_net = MultiStepLR(self.opt_net, milestones=[settings.l1, settings.l2], gamma=0.1)
        print(settings.total_step)
    ...
    while sess.step < settings.total_step+1:
        ...
        if sess.step % (settings.one_epoch * 10) == 0:
            ...
            logfile.write(
                ...
                'ssim  = ' + str(ssim_avg) + '\t'
                'pnsr  = ' + str(psnr_avg) + '\t'
                'save_checkpoint = '+'net_%d_epoch' % int(sess.step / settings.one_epoch)+'\t'
                '\n\n'
            )
            print((
                'step  = ' + str(sess.step) + '\t'
